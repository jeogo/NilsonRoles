...
import { DownloadIcon, Lightbulb } from 'lucide-react';
import { 
...
export default function Report() {
  ...
  useEffect(() => {
    if (radarChartRef.current && pieChartRef.current) {
      ...
      <ThemeProvider theme={theme}>
        <Box sx={{ minHeight: '100vh', background: 'linear-gradient(135deg, #f5f7ff 0%, #ffffff 100%)' }}>
          ...
          <Container maxWidth="lg" sx={{ pt: 10 }}>
            ...
            {/* Move off-screen instead of hiding, so html2canvas can render */}
            <Box
              ref={pieChartRef}
              sx={{
                position: 'absolute',
